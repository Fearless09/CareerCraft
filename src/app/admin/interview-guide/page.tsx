...
export default function AdminInterviewGuidePage() {
  ...
  const sectionsList =
    data?.sections?.filter((s) => s.type === "Interview") || [];
  ...
  const handleFormSubmit = async () => {
    ...
    try {
      ...
      addToast(
        `Interview guide section ${editId ? "updated" : "added"} successfully`,
        "success",
      );
      setFormData(null);
      ...
      const msg =
        err instanceof Error
          ? err.message
          : `Failed to ${editId ? "update" : "add"} interview guide section`;
      addToast(msg, "error");
    } finally {
  ...
  const handleMove = async (section: Guide, direction: "up" | "down") => {
    ...
    try {
      ...
      addToast("Interview guide section moved successfully", "success");
    } catch (err) {
      const msg =
        err instanceof Error
          ? err.message
          : "Failed to move interview guide section";
      addToast(msg, "error");
    } finally {
      setIsSubmitting(false);
    }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      addToast("Interview guide section deleted successfully", "success");
      setDeletingSection(null);
    } catch (err) {
      const msg =
        err instanceof Error
          ? err.message
          : "Failed to delete interview guide section";
      addToast(msg, "error");
    } finally {
      setIsDeleting(false);
    }
  ...
  return (
    <div className="space-y-8">
      ...
      <header className="flex flex-col items-center gap-4 text-center md:flex-row md:items-end md:justify-between md:text-left">
        <div>
          <h1 className="font-display flex items-center justify-center gap-2 text-3xl font-extrabold tracking-tight text-zinc-900 md:justify-start">
            <HelpCircle className="text-accent size-6" />
            Interview Preparation Guide
          </h1>
          <p className="mt-1 max-w-xl text-sm text-balance text-zinc-500">
            Manage the step-by-step reading modules shown on the public
        ...
        {isLoading ? (
          ...
          <div className="flex h-64 flex-col items-center justify-center rounded-2xl border border-dashed border-zinc-200 bg-zinc-50/50 p-8 text-center">
            ...
            <p className="mt-1 max-w-xs text-xs text-zinc-400">
              Click &quot;Add Section&quot; to create your first interview
              guide.
            </p>
          </div>
        ) : (
          <section className="space-y-4">
            {sectionsList.map((sect, index) => (
              <main
                key={sect.id}
                className="group transition-300 relative flex flex-col gap-4 rounded-2xl border border-zinc-200 bg-white p-6 shadow-xs hover:border-zinc-300 hover:shadow-md sm:flex-row sm:items-start sm:justify-between"
              >
                <div>
                  {sect.subtitle && (
                  ...
                  {sect.bullets && sect.bullets.length > 0 && (
                    <ul className="mt-4 space-y-2 border-t border-zinc-200 pt-4">
                      {sect.bullets.map((b, i) => (
                        ...
                        >
                          <span className="rounded border border-rose-200 bg-rose-50 p-0.5 text-rose-700">
      ...
      <DeleteModal
        ...
        modalTitle="Remove Guide Module"
      />
    </div>
  );
}
    ...
    <AddnUpdateModalWrapper isOpen={!!formData}>
      ...
      <div className="flex items-center justify-between border-b border-zinc-200 pb-2">
        <h3 className="font-display text-lg font-bold text-zinc-900">
          {editId ? "Modify Interview Guide" : "Add Interview Guide"}
        </h3>
        <button
          onClick={onCancel}
        ...
        >
          <div>
            ...
            <input
              ...
              placeholder="e.g. Types of Interviews"
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g. Navigating Different Formats"
              className="transition-300 w-full rounded-xl border border-zinc-200 bg-zinc-50 px-3 py-2.5 text-xs text-zinc-900 placeholder-zinc-400 focus:border-zinc-400 focus:outline-none"
              disabled={isSubmitting}
            />
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Provide detailed interview preparation content..."
          ...
          <div>
            ...
            <ul className="max-h-36 space-y-1 overflow-y-auto">
              {formData.bullets?.map((b, i) => (
                <li
                  key={i}
                  className="flex items-center justify-between rounded-lg border border-zinc-200 bg-zinc-50 p-2 text-[11px]"
                >
                  <span className="truncate pr-4 text-zinc-700">{b}</span>
                  <button
                    type="button"
                    onClick={() => onRemoveBullet(i)}
                    className="hover:text-rose-650 cursor-pointer text-zinc-400 transition-colors"
                    disabled={isSubmitting}
                  >
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label
                htmlFor="interview-cta-text"
                className="mb-2 block text-[10px] font-bold tracking-wider text-zinc-500 uppercase"
              >
                CTA Text (Optional)
              </label>
              <input
                id="interview-cta-text"
                type="text"
                value={formData.ctaText || ""}
                onChange={(e) => onChange({ ctaText: e.target.value })}
                placeholder="e.g. Practice Q&A"
                className="w-full rounded-xl border border-zinc-200 bg-zinc-50 px-3 py-2.5 text-xs text-zinc-900 placeholder-zinc-400 focus:border-zinc-400 focus:outline-none"
                disabled={isSubmitting}
              />
            </div>
            ...
            <div>
              <label
                htmlFor="interview-cta-link"
                className="mb-2 block text-[10px] font-bold tracking-wider text-zinc-500 uppercase"
              >
                CTA Link (Optional)
              </label>
              <input
                id="interview-cta-link"
                type="text"
